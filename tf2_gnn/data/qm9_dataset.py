...
class QM9GraphSample(GraphSample):
    """
     ...
     qm9数据集包括了13万有机分子的构成,空间信息及其对应的属性.
     它被广泛应用于各类数据驱动的分子属性预测方法的实验和对比。
    """
...
class QM9Dataset(GraphDataset[QM9GraphSample]):
    """
    QM9 Dataset class.
    QM9_nano.npz文件需要用numpy读取，其中包含三个字段：
    1.ID: 分子的id
    2.Atom：分子的原子构成，由一个原子序数的列表组成，如[6,1,1,1,1]
    3.Distance: 分子中原子的距离矩阵，以[6,1,1,1,1]分子为例，他的距离矩阵位一个5X5的矩阵
    4.U0:分子的能量属性，需要预测的值
    """
